---
title: "Óbitos por agressão"
output: pdf_document
---
```{r setup, cache=TRUE, warning=FALSE, message = FALSE, include=FALSE, echo=FALSE}
pacotes <- c("readr", "data.table", "dplyr", "ggplot2","ggthemes","stringr","devtools", "lubridate","read.dbc")
...
```{r get data, cache=TRUE, warning=FALSE, message = FALSE, echo=FALSE}
...
data.agressoes <- data.raw[,CAUSABAS := str_to_upper(CAUSABAS)
                           ...
                           # agressoes indeterminadas
                             ...
                             # agressoes especificas determinadas
                                 ...
                                 # data de obito
                                 ][,.(dt_obito = Data,CODMUNRES,agressoes,agressoesT,agressoesI,idx)]

# porcentagem de NA em dt_obito
print(paste("DO sem data de óbito:",data.agressoes[,round(mean(is.na(dt_obito))*100,3)],"%"))
...
agressoes.st <- agressoes[,.(
  ...
  # Agressoes Totais
  AgressõesTotais = sum(agressoesT,na.rm=T),
  # Agressoes indeterminadas
  AgressõesIndeterminadas = sum(agressoesI,na.rm=T)),by = .(year(dt_obito))] %>% 
  melt(id.vars='year',
       value.name = 'N',
...
```{r grafico agressoes, cache=TRUE, warning=FALSE, message = FALSE, echo=FALSE}
